...
pbaspect([1 1 1]);
maxAvstand = max(resultatmatris(:));
[row,col] = find(resultatmatris == maxAvstand);

optimalVinkel = startVinkel(row);
optimalVatten = vattenVolym(col);
display(maxAvstand)
display(optimalVatten)
display(optimalVinkel)
grid on;
